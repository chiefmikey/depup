# @depup/socket.io

> [socket.io](https://www.npmjs.com/package/socket.io) with all dependencies updated to latest versions.

Drop-in replacement. Same API. Fresher dependencies.

```bash
npm install @depup/socket.io
```

## Why?

Outdated transitive dependencies are the #1 source of npm security vulnerabilities. Most package maintainers don't bump their deps on every patch. DepUp does it automatically -- every 4 hours.

**Original version**: 4.8.3 | **DepUp version**: 4.8.3-depup.28 | **Updated**: 3/14/2026 | **Import test**: unknown

## What changed

| Dependency | Original | Updated |
|------------|----------|--------|
| accepts | `~1.3.4` | `^1.3.8` |
| cors | `~2.8.5` | `^2.8.6` |
| debug | `~4.4.1` | `^4.4.3` |
| engine.io | `~6.6.0` | `^6.6.6` |
| socket.io-adapter | `~2.5.2` | `^2.5.6` |
| socket.io-parser | `~4.2.4` | `^4.2.5` |


Something broken? [Report it](https://github.com/depup/npm/issues/new?title=Issue+with+@depup/socket.io&labels=bug).

## About DepUp

[DepUp](https://github.com/depup/npm) is an automated package factory that publishes dependency-bumped versions of 1000+ popular npm packages. [Request a package](https://github.com/depup/npm/issues/new?labels=package-request&title=Add+package:+PACKAGE_NAME&body=%23%23%23+Package+Name%0A%60PACKAGE_NAME%60) to be added.

This package inherits the license from [socket.io](https://www.npmjs.com/package/socket.io).