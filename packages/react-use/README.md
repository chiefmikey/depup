# @depup/react-use

> [react-use](https://www.npmjs.com/package/react-use) with all dependencies updated to latest versions.

Drop-in replacement. Same API. Fresher dependencies.

```bash
npm install @depup/react-use
```

## Why?

Outdated transitive dependencies are the #1 source of npm security vulnerabilities. Most package maintainers don't bump their deps on every patch. DepUp does it automatically -- every 4 hours.

**Original version**: 17.6.0 | **DepUp version**: 17.6.0-depup.0 | **Updated**: 3/22/2026 | **Import test**: failed

## What changed

| Dependency | Original | Updated |
|------------|----------|--------|
| @types/js-cookie | `^2.2.6` | `^3.0.6` |
| copy-to-clipboard | `^3.3.1` | `^3.3.3` |
| js-cookie | `^2.2.1` | `^3.0.5` |
| screenfull | `^5.1.0` | `^6.0.2` |
| throttle-debounce | `^3.0.1` | `^5.0.2` |
| tslib | `^2.1.0` | `^2.8.1` |


Something broken? [Report it](https://github.com/depup/npm/issues/new?title=Issue+with+@depup/react-use&labels=bug).

## About DepUp

[DepUp](https://github.com/depup/npm) is an automated package factory that publishes dependency-bumped versions of 1000+ popular npm packages. [Request a package](https://github.com/depup/npm/issues/new?labels=package-request&title=Add+package:+PACKAGE_NAME&body=%23%23%23+Package+Name%0A%60PACKAGE_NAME%60) to be added.

This package inherits the license from [react-use](https://www.npmjs.com/package/react-use).