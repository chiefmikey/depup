# @depup/sanity

> [sanity](https://www.npmjs.com/package/sanity) with all dependencies updated to latest versions.

Drop-in replacement. Same API. Fresher dependencies.

```bash
npm install @depup/sanity
```

## Why?

Outdated transitive dependencies are the #1 source of npm security vulnerabilities. Most package maintainers don't bump their deps on every patch. DepUp does it automatically -- every 4 hours.

**Original version**: 5.27.0 | **DepUp version**: 5.27.0-depup.0 | **Updated**: 5/26/2026 | **Import test**: failed

## What changed

| Dependency | Original | Updated |
|------------|----------|--------|
| @algorithm.ts/lcs | `^4.0.5` | `^4.0.6` |
| @date-fns/tz | `^1.4.1` | `^1.5.0` |
| @dnd-kit/modifiers | `^6.0.1` | `^9.0.0` |
| @dnd-kit/sortable | `^7.0.2` | `^10.0.0` |
| @isaacs/ttlcache | `^1.4.1` | `^2.1.5` |
| @mux/mux-player-react | `^3.10.2` | `^3.13.0` |
| @portabletext/editor | `^6.6.4` | `^7.0.1` |
| @portabletext/html | `^1.0.1` | `^1.0.2` |
| @portabletext/plugin-markdown-shortcuts | `^7.0.27` | `^8.0.1` |
| @portabletext/plugin-one-line | `^6.0.27` | `^7.0.1` |
| @portabletext/plugin-paste-link | `^3.0.27` | `^4.0.1` |
| @portabletext/plugin-typography | `^7.0.27` | `^8.0.1` |
| @portabletext/react | `^6.1.0` | `^6.2.0` |
| @portabletext/sanity-bridge | `^3.0.0` | `^3.1.0` |
| @sanity/client | `^7.21.0` | `^7.22.0` |
| @sanity/diff-patch | `^5.0.0` | `^6.0.0` |
| @sanity/image-url | `^2.0.3` | `^2.1.1` |
| @sanity/insert-menu | `^3.0.5` | `^3.0.7` |
| @sanity/migrate | `^6.1.1` | `^6.1.2` |
| @sanity/mutate | `^0.16.1` | `^0.18.0` |
| @sanity/presentation-comlink | `^2.0.1` | `^2.0.2` |
| @sanity/preview-url-secret | `^4.0.5` | `^4.0.6` |
| @sanity/sdk | `^2.8.0` | `^2.12.0` |
| @sanity/ui | `^3.1.14` | `^3.2.0` |
| @sentry/react | `^8.55.0` | `^10.54.0` |
| @tanstack/react-virtual | `^3.13.24` | `^3.13.26` |
| @xstate/react | `^6.0.0` | `^6.1.0` |
| classnames | `^2.2.5` | `^2.5.1` |
| date-fns | `^4.1.0` | `^4.3.0` |
| groq-js | `^1.29.0` | `^1.30.2` |
| i18next | `^26.2.0` | `^26.3.0` |
| isomorphic-dompurify | `2.26.0` | `^3.14.0` |
| motion | `^12.27.1` | `^12.40.0` |
| nanoid | `^3.3.11` | `^5.1.11` |
| path-to-regexp | `^6.3.0` | `^8.4.2` |
| player.style | `^0.1.9` | `^0.3.4` |
| react-is | `^19.2.4` | `^19.2.6` |
| semver | `^7.7.2` | `^7.8.1` |
| uuid | `^11.1.0` | `^14.0.0` |
| web-vitals | `^5.1.0` | `^5.2.0` |
| xstate | `^5.25.1` | `^5.31.1` |


Something broken? [Report it](https://github.com/depup/npm/issues/new?title=Issue+with+@depup/sanity&labels=bug).

## About DepUp

[DepUp](https://github.com/depup/npm) is an automated package factory that publishes dependency-bumped versions of 1000+ popular npm packages. [Request a package](https://github.com/depup/npm/issues/new?labels=package-request&title=Add+package:+PACKAGE_NAME&body=%23%23%23+Package+Name%0A%60PACKAGE_NAME%60) to be added.

This package inherits the license from [sanity](https://www.npmjs.com/package/sanity).