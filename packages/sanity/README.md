# @depup/sanity

> [sanity](https://www.npmjs.com/package/sanity) with all dependencies updated to latest versions.

Drop-in replacement. Same API. Fresher dependencies.

```bash
npm install @depup/sanity
```

## Why?

Outdated transitive dependencies are the #1 source of npm security vulnerabilities. Most package maintainers don't bump their deps on every patch. DepUp does it automatically -- every 4 hours.

**Original version**: 5.17.1 | **DepUp version**: 5.17.1-depup.1 | **Updated**: 3/19/2026 | **Import test**: failed

## What changed

| Dependency | Original | Updated |
|------------|----------|--------|
| @dnd-kit/modifiers | `^6.0.1` | `^9.0.0` |
| @dnd-kit/sortable | `^7.0.2` | `^10.0.0` |
| @isaacs/ttlcache | `^1.4.1` | `^2.1.4` |
| @mux/mux-player-react | `^3.10.2` | `^3.11.5` |
| @portabletext/editor | `^6.4.0` | `^6.5.1` |
| @portabletext/plugin-markdown-shortcuts | `^7.0.19` | `^7.0.21` |
| @portabletext/plugin-one-line | `^6.0.19` | `^6.0.21` |
| @portabletext/plugin-paste-link | `^3.0.19` | `^3.0.21` |
| @portabletext/plugin-typography | `^7.0.19` | `^7.0.21` |
| @sanity/cli | `^6.1.5` | `^6.1.8` |
| @sanity/diff-patch | `^5.0.0` | `^6.0.0` |
| @sanity/sdk | `2.1.2` | `^2.8.0` |
| @sanity/telemetry | `^0.8.1` | `^0.9.0` |
| @sentry/react | `^8.55.0` | `^10.44.0` |
| @tanstack/react-virtual | `^3.13.18` | `^3.13.23` |
| @xstate/react | `^6.0.0` | `^6.1.0` |
| classnames | `^2.2.5` | `^2.5.1` |
| i18next | `^25.8.17` | `^25.8.19` |
| isomorphic-dompurify | `2.26.0` | `^3.5.1` |
| lodash-es | `^4.17.22` | `^4.17.23` |
| motion | `^12.27.1` | `^12.38.0` |
| nanoid | `^3.3.11` | `^5.1.7` |
| path-to-regexp | `^6.3.0` | `^8.3.0` |
| player.style | `^0.1.9` | `^0.3.1` |
| react-i18next | `15.6.1` | `^16.5.8` |
| semver | `^7.7.2` | `^7.7.4` |
| uuid | `^11.1.0` | `^13.0.0` |
| xstate | `^5.25.1` | `^5.28.0` |


Something broken? [Report it](https://github.com/depup/npm/issues/new?title=Issue+with+@depup/sanity&labels=bug).

## About DepUp

[DepUp](https://github.com/depup/npm) is an automated package factory that publishes dependency-bumped versions of 1000+ popular npm packages. [Request a package](https://github.com/depup/npm/issues/new?labels=package-request&title=Add+package:+PACKAGE_NAME&body=%23%23%23+Package+Name%0A%60PACKAGE_NAME%60) to be added.

This package inherits the license from [sanity](https://www.npmjs.com/package/sanity).