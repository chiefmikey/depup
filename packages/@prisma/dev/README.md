# @depup/prisma__dev

> [@prisma/dev](https://www.npmjs.com/package/@prisma/dev) with all dependencies updated to latest versions.

Drop-in replacement. Same API. Fresher dependencies.

```bash
npm install @depup/prisma__dev
```

## Why?

Outdated transitive dependencies are the #1 source of npm security vulnerabilities. Most package maintainers don't bump their deps on every patch. DepUp does it automatically -- every 4 hours.

**Original version**: 0.24.5 | **DepUp version**: 0.24.5-depup.2 | **Updated**: 4/15/2026 | **Import test**: failed

## What changed

| Dependency | Original | Updated |
|------------|----------|--------|
| @electric-sql/pglite | `0.4.1` | `^0.4.4` |
| @electric-sql/pglite-socket | `0.1.1` | `^0.1.4` |
| @electric-sql/pglite-tools | `0.3.1` | `^0.3.4` |
| @hono/node-server | `1.19.11` | `^1.19.14` |
| @prisma/get-platform | `7.2.0` | `^7.7.0` |
| @prisma/query-plan-executor | `7.2.0` | `^7.7.0` |
| @prisma/streams-local | `0.1.3` | `^0.1.4` |
| foreground-child | `3.3.1` | `^4.0.3` |
| hono | `^4.12.8` | `^4.12.14` |
| remeda | `2.33.4` | `^2.33.7` |
| std-env | `3.10.0` | `^4.1.0` |
| valibot | `1.2.0` | `^1.3.1` |


Something broken? [Report it](https://github.com/depup/npm/issues/new?title=Issue+with+@depup/prisma__dev&labels=bug).

## About DepUp

[DepUp](https://github.com/depup/npm) is an automated package factory that publishes dependency-bumped versions of 1000+ popular npm packages. [Request a package](https://github.com/depup/npm/issues/new?labels=package-request&title=Add+package:+PACKAGE_NAME&body=%23%23%23+Package+Name%0A%60PACKAGE_NAME%60) to be added.

This package inherits the license from [@prisma/dev](https://www.npmjs.com/package/@prisma/dev).